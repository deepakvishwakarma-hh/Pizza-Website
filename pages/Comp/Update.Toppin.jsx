...
export default function UpdateToppins({ toNext, updateToppins }) {
    ...
    const addToppinsButtonList = ToppinsData.map((v, i) => {
        return (
            <button key={v.name} className={Styles.ToppinsButton} onClick={() => { addTopins([...toppins, v.name]); setShowAleart(false) }
            }>{v.name}</button>
        )
    })
    const addedToppinsButtonList = [...new Set(toppins)].map((v, i) => {
        return (
            <li key={v} className={Styles.listedItems}><h3>{v}</h3> <button onClick={() => {
